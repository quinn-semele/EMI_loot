...
public class EntityTypePredicateParser{
    ...
    public static Text parseEntityTypePredicate(EntityTypePredicate predicate){
        ...
        if (tag.isPresent()) {
            return Text.translatable("emi_loot.entity_predicate.type_tag", tag.get().id());
        } else if (registryEntryList.size() == 1) {
            return Text.translatable("emi_loot.entity_predicate.type_single", registryEntryList.get(0).value().getName().getString());
        }
